...
export function ProjectDetailModal({
  ...
  // DOM Refs for GSAP precision animation control
  ...
  const handleClose = useCallback(() => {
    ...
    if (headerRef.current) {
      tl.to(headerRef.current, { opacity: 0, y: -20, duration: 0.22 }, 0);
    }
    if (navLeftRef.current) {
      tl.to(navLeftRef.current, { opacity: 0, scale: 0.85, duration: 0.18 }, 0);
    }
    if (navRightRef.current) {
      tl.to(navRightRef.current, { opacity: 0, scale: 0.85, duration: 0.18 }, 0);
    }
    if (modalContainerRef.current) {
      tl.to(
        modalContainerRef.current,
        { opacity: 0, scale: 0.94, y: 20, duration: 0.26, ease: "power3.in" },
        0
      );
    }
    if (backdropRef.current) {
      tl.to(backdropRef.current, { opacity: 0, duration: 0.28, ease: "power2.inOut" }, "-=0.12");
    }
  }, [isClosing, onClose]);

  // ─── GSAP Switch / Navigate Sequence ──────────────────────────────────────
  const handleNavigate = useCallback(
    (slug: string, dir: number) => {
      if (isNavigating.current || isClosing) return;
      ...
      // Smooth slide and fade out current active content in navigation direction
      if (contentGridRef.current) {
        gsap.to(contentGridRef.current, {
          x: dir * -50,
          opacity: 0.15,
          scale: 0.98,
          duration: 0.18,
  ...
  }, [isOpen, project, handleClose, handleNavigate]);

  // ─── GSAP Intro Sequence on Modal Open ───────────────────────────────────
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 3. Floating Navigation Arrows
      const arrows = [navLeftRef.current, navRightRef.current].filter(Boolean);
      if (arrows.length > 0) {
        tl.fromTo(
          arrows,
          { opacity: 0, scale: 0.8 },
      ...
      // 4. Modal Container expansion & subtle zoom
      if (modalContainerRef.current) {
        tl.fromTo(
          modalContainerRef.current,
      ...
      if (mediaRef.current) {
        tl.fromTo(
          ...
          { opacity: 1, scale: 1, filter: "blur(0px)", duration: 0.46 },
          "-=0.32"
        );
      }

      // 7. Staggered details
      const staggerEls = modalRootRef.current?.querySelectorAll(".modal-stagger-item");
  ...
  // ─── GSAP Project Content Swap Sequence (when navigating) ────────────────
  useEffect(() => {
    if (!isOpen || !project) return;
    ...
    if (contentGridRef.current) {
      gsap.fromTo(
        ...
        { x: direction * 50, opacity: 0.15, scale: 0.98 },
    ...
    if (mediaRef.current) {
      gsap.fromTo(
        ...
        { filter: "brightness(1) blur(0px)", duration: 0.42, ease: "power2.out" }
      );
    }
  }, [project?.slug, direction, isOpen]);
  ...
  return (
    ...
    >
      {/* ── Backdrop with Dark Ambient Blur ── */}
      <div
        ref={backdropRef}
        onClick={handleClose}
        className="absolute inset-0 bg-[#050505]/92 backdrop-blur-xl cursor-pointer"
        aria-hidden="true"
      />
      ...
      <div
        ref={headerRef}
        className="absolute top-14 sm:top-16 left-0 right-0 z-50 px-4 sm:px-8 py-2.5 flex items-center justify-between border-b border-white/10 bg-[#050505]/95 backdrop-blur-md pointer-events-none"
      >
        {/* Left Telemetry */}
        <div className="flex items-center gap-3 font-mono text-[0.6rem] tracking-[0.2em] text-white/40 pointer-events-auto select-none">
          ...
          <span className="hidden sm:inline text-white/40">SIGNAL_SECURE</span>
        </div>
        ...
        <button
          ...
          className="pointer-events-auto group px-4 py-1.5 font-mono text-[0.68rem] tracking-[0.2em] uppercase transition-all duration-200 flex items-center gap-2 cursor-pointer shadow-lg hover:border-[#FFAA00] hover:bg-[#FFAA00]/15"
          style={{
        ...
        {/* Right Status */}
        <div className="flex items-center gap-2 font-mono text-[0.6rem] tracking-[0.2em] text-white/40 pointer-events-auto select-none">
      ...
      {previous && (
        <button
          ref={navLeftRef}
          onClick={() => handleNavigate(previous.slug, -1)}
          onMouseEnter={() => playSound("hover")}
          aria-label={`Previous project: ${previous.title}`}
          className="hidden md:flex absolute left-3 lg:left-6 top-1/2 -translate-y-1/2 z-50 w-11 h-11 items-center justify-center border border-white/15 hover:border-[#FFAA00] bg-[#0c0c0c]/90 hover:bg-[#FFAA00]/15 text-white/70 hover:text-[#FFAA00] transition-all duration-200 shadow-2xl backdrop-blur-md cursor-pointer group"
        >
          <svg
            width="18"
            height="18"
      ...
      {next && (
        <button
          ref={navRightRef}
          onClick={() => handleNavigate(next.slug, 1)}
          onMouseEnter={() => playSound("hover")}
          aria-label={`Next project: ${next.title}`}
          className="hidden md:flex absolute right-3 lg:right-6 top-1/2 -translate-y-1/2 z-50 w-11 h-11 items-center justify-center border border-white/15 hover:border-[#FFAA00] bg-[#0c0c0c]/90 hover:bg-[#FFAA00]/15 text-white/70 hover:text-[#FFAA00] transition-all duration-200 shadow-2xl backdrop-blur-md cursor-pointer group"
        >
          <svg
            width="18"
            height="18"
      ...
      {/* ── Full Modal Content Window with Custom Theme Scrollbar & Sticky Sidebar ── */}
      <div
        ref={modalContainerRef}
        className="relative z-40 w-full max-w-6xl max-h-[calc(100vh-7.5rem)] mt-24 sm:mt-28 overflow-y-auto px-4 sm:px-8 py-6 select-text custom-scrollbar"
      >
        <div ref={contentGridRef} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* ══════════════════════════════════════════════════════════════
              4-COLUMN LEFT METADATA SIDEBAR (STICKY ON DESKTOP)
          ══════════════════════════════════════════════════════════════ */}
          <aside ref={sidebarRef} className="lg:col-span-4 lg:sticky lg:top-0 self-start flex flex-col gap-6">
            <div
            ...
            >
              ...
              <div className="flex flex-col divide-y divide-white/5 font-mono text-xs">
                {/* YEAR */}
                <div className="py-2.5 flex items-center justify-between modal-stagger-item">
              ...
              {/* TAGS SECTION */}
              <div className="mt-5 pt-4 border-t border-white/10 modal-stagger-item">
              ...
              <div className="mt-6 pt-4 border-t border-white/10 flex flex-col gap-2.5 modal-stagger-item">
                {project.links.liveDemo ? (
                  <a
                    ...
                    className="w-full text-center py-2.5 font-mono text-xs tracking-[0.2em] uppercase font-bold bg-[#FFAA00] hover:bg-[#ffbe33] text-[#050505] transition-all duration-200 shadow-lg cursor-pointer"
                  >
                    ACCESS SITE
                  </a>
                ...
                {project.links.github && (
                  <a
                    ...
                    onMouseEnter={() => playSound("hover")}
                    onClick={() => playSound("open")}
                    className="w-full text-center py-2 font-mono text-[0.68rem] tracking-[0.18em] uppercase border border-white/15 hover:border-[#FFAA00] text-white/70 hover:text-[#FFAA00] bg-white/[0.02] hover:bg-[#FFAA00]/5 transition-all duration-200 cursor-pointer"
                  >
                    VIEW SOURCE REPO
                  </a>
                )}
          ...
          <main className="lg:col-span-8 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-2 modal-stagger-item">
            ...
            <div ref={mediaRef} className="project-media-frame group overflow-hidden bg-[#0a0a0a] rounded-xs">
            ...
            {project.caseStudy && (
              <div className="flex flex-col gap-6 modal-stagger-item">
                {/* Technical Analysis */}
            ...
            <div className="pt-6 border-t border-white/10 flex items-stretch gap-3 modal-stagger-item">
              {previous ? (
                <button
                  onClick={() => handleNavigate(previous.slug, -1)}
                  onMouseEnter={() => playSound("hover")}
                  className="flex-1 p-3 sm:p-4 border border-white/10 hover:border-[#FFAA00]/50 bg-white/[0.02] hover:bg-[#FFAA00]/5 text-left transition-all duration-200 group cursor-pointer"
                >
              ...
              {next ? (
                <button
                  onClick={() => handleNavigate(next.slug, 1)}
                  onMouseEnter={() => playSound("hover")}
                  className="flex-1 p-3 sm:p-4 border border-white/10 hover:border-[#FFAA00]/50 bg-white/[0.02] hover:bg-[#FFAA00]/5 text-right transition-all duration-200 group cursor-pointer"
                >
